    ...
    while (command != "Quit") {
        if (command == "Initialize") {
            ...
            try {
                ...
                outFile << command << ": " << date.GetMonthAsString() << " " << date.GetDay() << ", " << date.GetYear()
                        << endl;
            } catch (string msg) {
                outFile << msg << endl;
            }
            ...
            switch (date.ComparedTo(date2)) {
                case LESS:
                    ...
                    break;
                case GREATER:
                    outFile << date2.GetMonthAsString() << " " << date2.GetDay() << ", " << date2.GetYear();
                    outFile << " comes before ";
                    outFile << date.GetMonthAsString() << " " << date.GetDay() << ", " << date.GetYear() << endl;
                    break;
